...
const RekamMedisItem: React.FC<RekamMedisItemProps> = ({ nama, keluhanList, onLihatClick, onTambahClick }) => (
  <Card sx={{ mb: 2, backgroundColor: keluhanList.length === 0 ? '#ffe0b2' : '#f5f5f5' }}>
    <CardContent sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <Box>
        <Typography variant="h6">{nama}</Typography>
        <Typography variant="body2" color="text.secondary">
          {keluhanList.length === 0 ? '*Belum ada rekam medis' : keluhanList.join(', ')}
        </Typography>
      </Box>
      <Box>
        {keluhanList.length === 0 ? (
          <Button variant="contained" color="primary" startIcon={<AddIcon />} size="small" onClick={onTambahClick}>
            Tambahkan
          </Button>
        ) : (
          <Button variant="outlined" color="primary" startIcon={<VisibilityIcon />} size="small" onClick={onLihatClick}>
            Lihat
          </Button>
        )}
      </Box>
    ...
    if (formNama) {
      const updatedData = rekamMedisData.map((item) => {
        if (item.nama === formNama) {
          return { ...item, keluhanList: [...item.keluhanList, formData.keluhan] };
        }
        return item;
      });
